...
class Listen_():
        ...
        try:
            ...
            for i, rule_ in enumerate(rules_):
                if "#" in str(rule_) or len(str(rule_)) <= 10:
                    ...
                    Thread(target=self.start_listening, args=(typ, src_ip, src_pt, dst_ip, dst_pt, flags_, flags_ui)).start()
    ...
    # CONFIG RULES
    def start_snrokl(self, rules_file, flags_):
        self.get_rules(rules_file, flags_)
...
if __name__=="__main__":
    L = Listen_()
    if len(sys.argv) > 1:
        L.start_snrokl(sys.argv[1], sys.argv[2])
    else:
        print("REQUIRES: RULES_FILE & FLAGS")
        sys.exit()
